...
// NEW: Force-pull all data from Firebase using direct get() calls
// This is more reliable on mobile than listener re-attachment
export async function forcePullFromCloud() {
    if (!auth.currentUser) throw new Error('Not authenticated');
    
    const paths = [
        'appState/transactions_public',
        'appState/transactions_private',
        'appState/houses',
        'appState/categories',
        'appState/budgets',
        'appState/recurringTemplates',
        'appState/payers',
        'appState/currency',
        'appState/electricRate',
        'appState/lastUpdated',
        'appState/lastWriteId'
    ];
    
    const results = {};
    // Fetch all paths in parallel for speed
    const snapshots = await Promise.all(paths.map(p => get(ref(database, p))));
    paths.forEach((p, i) => {
        const key = p.replace('appState/', '');
        results[key] = snapshots[i].val() || null;
    });
    
    // Build full state from results
    const mergedTxs = {
        ...(results.transactions_public || {}),
        ...(results.transactions_private || {})
    };
    
    const fullState = {
        transactions: Object.values(mergedTxs).sort((a, b) => {
            const dateA = new Date(a.createdAt || a.date).getTime();
            const dateB = new Date(b.createdAt || b.date).getTime();
            return dateB - dateA;
        }),
        houses: results.houses ? Object.values(results.houses).filter(Boolean) : [],
        categories: results.categories || { expense: [], groceries: [] },
        budgets: results.budgets || {},
        recurringTemplates: results.recurringTemplates || [],
        payers: results.payers || [],
        currency: results.currency || '₹',
        electricRate: results.electricRate || 8,
        lastUpdated: results.lastUpdated || 0,
        lastWriteId: results.lastWriteId || null,
        _cloudWriteId: results.lastWriteId || null,
        _cloudLastUpdated: results.lastUpdated || 0
    };
...
window.showLoginUI = (isMandatory = false) => {
    ...
    modal.innerHTML = `
        ...
        <div class="glass-card" style="width:90%;max-width:340px;padding:24px;text-align:center;animation:fbModalPop 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);">
            <div style="font-size:2.5rem;margin-bottom:8px;">💰</div>
            <h3 id="fbLoginTitle" style="margin-top:0;margin-bottom:4px;">${isMandatory ? 'Home Finlytics' : 'Cloud Sync'}</h3>
            <p id="fbLoginDesc" style="font-size:0.85rem;color:var(--text-secondary);margin-bottom:20px;">${isMandatory ? 'Sign in to access your dashboard.' : 'Sign in to enable real-time sync across all your devices.'}</p>
            
            <!-- Google Sign-In — Primary -->
            <button id="fbGoogleSignInBtn" class="btn btn-primary btn-full" style="display:flex;align-items:center;justify-content:center;gap:10px;padding:12px;font-size:0.95rem;font-weight:600;margin-bottom:16px;border-radius:8px;">
            ...
            <p style="font-size:0.75rem;color:var(--text-secondary);margin-bottom:12px;">Recommended — one account across all devices</p>
            
            <!-- Divider -->
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                <div style="flex:1;height:1px;background:var(--divider);"></div>
                <span style="font-size:0.75rem;color:var(--text-tertiary);">or</span>
                <div style="flex:1;height:1px;background:var(--divider);"></div>
            </div>
            
            <!-- Email/Password — Collapsible secondary -->
            <div id="fbEmailSection" style="display:none;">
                <input type="email" id="fbLoginEmail" placeholder="Email Address" class="form-input" style="margin-bottom:10px;width:100%;box-sizing:border-box;">
            ...
            <button id="fbShowEmailBtn" class="btn btn-sm btn-secondary" style="width:100%;font-size:0.8rem;">Sign in with Email & Password</button>
    ...
    const emailSection = document.getElementById('fbEmailSection');
    const showEmailBtn = document.getElementById('fbShowEmailBtn');

    // Toggle email/password section
    showEmailBtn.addEventListener('click', () => {
        ...
        showEmailBtn.textContent = visible ? 'Sign in with Email & Password' : 'Hide Email Sign-In';
    });
